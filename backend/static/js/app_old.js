...
const app = {
        ...
        container.innerHTML = specials.map(item => `
            <div class="glass special-card" onclick="app.openDetails(${item.id})">
                ...
                <div class="special-content">
                    ...
                    <p style="color: var(--accent-gold); font-weight: 800;">${item.price.toLocaleString()} s┼Нm</p>
                </div>
            </div>
        `).join('');
    },
            ...
            return `
                <div class="glass product-card">
                    ...
                    <div class="product-body">
                        <div class="product-price">${item.price.toLocaleString()} s┼Нm</div>
    ...
    openDetails: (id) => {
        ...
        body.innerHTML = `
            ...
            <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 40px;">
                <span style="font-size: 1.8rem; font-weight: 900; color: var(--accent-gold);">${item.price.toLocaleString()} s┼Нm</span>
    ...
    updateCartUI: () => {
        ...
        document.getElementById('cart-total').innerText = `${total.toLocaleString()} s┼Нm`;
        document.getElementById('final-price').innerText = `${total.toLocaleString()} s┼Нm`;
            ...
            container.innerHTML = orders.map(o => `
                <div class="glass" style="grid-column: 1/-1; padding: 20px; margin-bottom: 12px;">
                    ...
                    <div style="text-align: right; font-weight: 900; margin-top: 8px; font-size: 1.1rem; color: var(--accent-gold);">
                        ${o.total_price.toLocaleString()} s┼Нm
                    </div>
                </div>
            `).join('');
        } catch (e) {
